...
const SHAPE_COLORS: Record<string,string> = {
  C: '#e0735c', A: '#4a90d9', G: '#5cb87a', E: '#c77fd9', D: '#d9b54a',
};
...
export default function CagedTab() {
  const {
    ...
    soundOn, setSoundOn,
    handleNoteClick,
  } = useCagedExplorer() as any;

  const fretWrapRef = useRef<HTMLDivElement | null>(null);
  const [fretWidth, setFretWidth] = useState<number>(760);
  useEffect(() => {
  ...
  return (
    <div className="caged-layout">
      ...
      <div className="card caged-fretboard-card">
        <div className="card-header"><span className="card-label">{root} major across the neck</span></div>
        <div ref={fretWrapRef} style={{ width: '100%' }}>
          <Fretboard width={Math.max(320, fretWidth)} dots={dots} showNoteNames onFretClick={handleNoteClick} />
        </div>

        <div className="legend" style={{ paddingTop: 10 }}>
          <div className="legend-item"><div className="legend-dot" style={{ background: 'var(--accent)' }} />Root note</div>
          <div className="legend-item"><div className="legend-dot" style={{ background: 'var(--chordtone, #4a90d9)' }} />Chord tone (3rd/5th)</div>
          <div className="legend-item"><div className="legend-dot" style={{ background: 'var(--text-2)' }} />Scale passing note</div>
        </div>
      </div>

      <div className="card caged-chain-card">
        <div className="card-header"><span className="card-label">How the shapes chain up the neck</span></div>
        <div className="caged-chain">
          {orderedShapes.map(({ key, minFret }: any, i: number) => (
            <div key={key} className="chain-item">
              <button
                className="chain-shape-btn"
                style={{ borderColor: SHAPE_COLORS[key], background: activeShapes.has(key) ? SHAPE_COLORS[key] + '22' : 'transparent', color: SHAPE_COLORS[key] }}
        ...
        <p className="caged-explainer">Each shape is named after the open chord it resembles. Together the five shapes form one continuous pattern that repeats every octave (12 frets). Click a shape above to isolate it, or toggle multiple shapes to see how they overlap.</p>
      </div>
    </div>
  );
}
