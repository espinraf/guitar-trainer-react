// ExplorerTab.jsx — Fretboard scale/note explorer
...
function buildExplorerDots(rootNote, scaleName) {
  ...
  for (let s = 0; s < STRING_COUNT; s++) {
    for (let f = 0; f <= FRET_COUNT; f++) {
      ...
      dots.push({
        str: s, fret: f,
        type: noteClass === rootMidi ? 'root' : 'scale',
        label: note,
      });
    }
  }
      ...
      <div className="card" style={{ padding: '1.5rem' }}>
        <div className="card-header">
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <select value={root} onChange={e => setRoot(e.target.value)} aria-label="Root note">
              {CHROMATIC.map(n => <option key={n}>{n}</option>)}
            </select>
            <select value={scale} onChange={e => setScale(e.target.value)} aria-label="Scale type">
